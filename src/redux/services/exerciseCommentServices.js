...
export function getAnExerciseComments(exerciseId, page) {
  return dispatch => {
    ...
    authRequest.get(`/exercises/${exerciseId}/comments?page=${page}&sort=submitDtm,desc`)
      .then(response_1 => {
        let result_1 = response_1.data;
        let IDarr = '';
        result_1.exerciseCommentDTOs.map(item => IDarr += item.id + ",") //tao ra mang id moi
        authRequest.get(`/exercises/comments/statistics?commentIDs=${IDarr}`)
          .then(response_2 => {
            //merge summary array and statistic array
            let result_2 = [];

            for (let i = 0; i < result_1.exerciseCommentDTOs.length; i++) {
              result_2.push({
                ...result_1.exerciseCommentDTOs[i],
                ...(response_2.data.find((itmInner) => itmInner.id === result_1.exerciseCommentDTOs[i].id)),
              });
            }
            let actionIDarr = IDarr.length > 1 ? IDarr.substring(0, IDarr.length - 1) : IDarr;

            authRequest.get(`/exercises/comments/actionAvailable?exerciseCommentIDs=${actionIDarr}`).then(response_3 => {
              let finalResult = [];
              ...
              dispatch(get_AnExerciseCommentsSuccess({ exerciseCommentDTOs: finalResult, totalPages: result_1.totalPages, totalElements: result_1.totalElements }))
...
export function editAnExerciseComment(commentId, data) {
  return dispatch => {
    ...
    authRequest.put(`/exercises/comments/${commentId}`, JSON.stringify(data))
      .then(response => {
...
export function getCommentReportReasons() {
  return dispatch => {
    ...
    authRequest.get(`/exercises/comments/report`).then(response =>
...
export function reportAnExerciseComment(id, reason) { //
  return dispatch => {
    ...
    authRequest.post(`/exercises/comments/${id}/report`, JSON.stringify(reason))
      .then(response => {
...
export function createAnExerciseComment(exerciseId, content) {
  return dispatch => {
    ...
    authRequest.post(`/exercises/` + exerciseId + `/comments`, JSON.stringify(content))
      .then(response => {
...
export function likeAnExerciseComment(commentId) { //maybe use modal later
  return dispatch => {
    ...
    authRequest.post(`/exercises/comments/${commentId}/likeStatus`)
      .then(response => {
...
export function unLikeAnExerciseComment(commentId) { //maybe use modal later
  return dispatch => {
    ...
    authRequest.delete(`/exercises/comments/${commentId}/likeStatus`)
      .then(response => {
...
export function deleteAnExerciseComment(commentId) { //maybe use modal later
  return dispatch => {
    ...
    authRequest.delete(`/exercises/comments/${commentId}`).then(response => {
...
export function createReply(prCommentId, content) {
  return dispatch => {
    dispatch(create_AnExerciseCommentReset());
    authRequest.post(`/exercises/comments/${prCommentId}`, JSON.stringify(content))
      .then(response => {
        dispatch(create_AnExerciseCommentSuccess(response.data.id));
      })
      .catch(error => {
