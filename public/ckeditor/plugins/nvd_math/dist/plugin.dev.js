...
(function () {
  CKEDITOR.plugins.add('nvd_math', {
    ...
    init: function init(editor) {
      ...
      ;
      editor.addContentsCss(pluginDirectory + '/styles/styles.css'); // Load Additional CSS 
      ...
      var pluginCmd = 'nvd_mathDialog'; // Add the link and unlink buttons.
      ...
      editor.widgets.add('nvd_math', {
        ...
        styleToAllowedContentRules: function styleToAllowedContentRules(style) {
        ...
        init: function init() {
          var iframe = this.parts.span.getChild(0); // Check if span contains iframe and create it otherwise.
          ...
          this.once('ready', function () {
            ...
            if (CKEDITOR.env.ie) iframe.setAttribute('src', CKEDITOR.plugins.nvd_math.fixSrc);
        ...
        upcast: function upcast(el, data) {
          ...
          data.math = CKEDITOR.tools.htmlDecode(el.children[0].value); // Add style display:inline-block to have proper height of widget wrapper and mask.

          var attrs = el.attributes;
          if (attrs.style) attrs.style += ';display:inline-block';else attrs.style = 'display:inline-block'; // Add attribute to prevent deleting empty span in data processing.
        ...
        downcast: function downcast(el) {
          el.children[0].replaceWith(new CKEDITOR.htmlParser.text(CKEDITOR.tools.htmlEncode(this.data.math))); // Remove style display:inline-block.
      ...
      CKEDITOR.dialog.add(pluginCmd, this.path + "dialogs/nvd_math.js"); // add context-menu entry
      ...
      }, null, null, 1); // Add MathJax script to page preview.

      editor.on('contentPreview', function (evt) {
        console.log('content previewed');
        evt.data.dataValue = evt.data.dataValue.replace(/<\/head>/, '<script src="' + CKEDITOR.getUrl(editor.config.mathJaxLib) + '"></script></head>');
      }); //MathJax
  ...
  */
  ...
  CKEDITOR.plugins.nvd_math.fixSrc = // In Firefox src must exist and be different than about:blank to emit load event.
  CKEDITOR.env.gecko ? 'javascript:true' : // jshint ignore:line
  // Support for custom document.domain in IE.
  CKEDITOR.env.ie ? 'javascript:' + // jshint ignore:line
  'void((function(){' + encodeURIComponent('document.open();' + '(' + CKEDITOR.tools.fixDomain + ')();' + 'document.close();') + '})())' : // In Chrome src must be undefined to emit load event.
  ...
  if (!(CKEDITOR.env.ie && CKEDITOR.env.version === 8)) {
    CKEDITOR.plugins.nvd_math.frameWrapper = function (iFrame, editor) {
      ...
      loadedHandler = CKEDITOR.tools.addFunction(function () {
        ...
        isInit = true;
        if (newValue) update(); // Private! For test usage only.
      ...
      updateDoneHandler = CKEDITOR.tools.addFunction(function () {
        ...
        editor.fire('unlockSnapshot'); // Private! For test usage only.

        CKEDITOR.fire('mathJaxUpdateDone', iFrame); // If value changed in the meantime update it again.
      ...
      function load() {
        doc = iFrame.getFrameDocument();
        if (doc.getById('preview')) return; // Because of IE9 bug in a src attribute can not be javascript
        ...
        doc.write('<!DOCTYPE html>' + '<html>' + '<head>' + '<meta charset="utf-8">' + '<script type="text/x-mathjax-config">' + // MathJax configuration, disable messages.
        'MathJax.Hub.Config( {' + 'showMathMenu: false,' + 'messageStyle: "none"' + '} );' + // Get main CKEDITOR form parent.
        'function getCKE() {' + 'if ( typeof window.parent.CKEDITOR == \'object\' ) {' + 'return window.parent.CKEDITOR;' + '} else {' + 'return window.parent.parent.CKEDITOR;' + '}' + '}' + // Run MathJax.Hub with its actual parser and call callback function after that.
        // Because MathJax.Hub is asynchronous create MathJax.Hub.Queue to wait with callback.
        'function update() {' + 'MathJax.Hub.Queue(' + '[ \'Typeset\', MathJax.Hub, this.buffer ],' + 'function() {' + 'getCKE().tools.callFunction( ' + updateDoneHandler + ' );' + '}' + ');' + '}' + // Run MathJax for the first time, when the script is loaded.
        // Callback function will be called then it's done.
        'MathJax.Hub.Queue( function() {' + 'getCKE().tools.callFunction(' + loadedHandler + ');' + '} );' + '</script>' + // Load MathJax lib.
        '<script src="' + editor.config.mathJaxLib + '"></script>' + '</head>' + '<body style="padding:0;margin:0;background:transparent;overflow:hidden">' + '<span id="preview"></span>' + // Render everything here and after that copy it to the preview.
        '<span id="buffer" style="display:none"></span>' + '</body>' + '</html>');
      ...
      function update() {
        ...
        value = newValue;
        editor.fire('lockSnapshot');
        buffer.setHtml(value); // Set loading indicator.
        ...
        editor.fire('unlockSnapshot'); // Run MathJax.
    ...
    CKEDITOR.plugins.nvd_math.frameWrapper = function (iFrame, editor) {
      iFrame.getFrameDocument().write('<!DOCTYPE html>' + '<html>' + '<head>' + '<meta charset="utf-8">' + '</head>' + '<body style="padding:0;margin:0;background:transparent;overflow:hidden">' + '<span style="white-space:nowrap;" id="tex"></span>' + '</body>' + '</html>');
      return {
        setValue: function setValue(value) {
